...
public class SmallGameState {
    ...
    private final MrxSmallPlayer mrX;
    private final ImmutableList<DetSmallPlayer> detectives;

    SmallGameState(Integer logNumber, MrxSmallPlayer mrX, ImmutableList<DetSmallPlayer> detectives){
    ...
    public MrxSmallPlayer mrX() {return this.mrX;}

    public ImmutableList<DetSmallPlayer> detectives() {return this.detectives;}

    AbstractSmallPlayer getPlayer(int id) {
        if (id == 0) {return this.mrX;}
        else {
            for (DetSmallPlayer player : detectives) {
                if (player.id() == id)
                    return player;
            }
        }
    ...
    private Boolean isTrapped(Boolean mrX) { // optimise
        ...
        if (mrX) {
            for (Integer neighbour : Setup.getInstance().graph.adjacentNodes(this.mrX.location())) {   //for each of their neighbouring nodes
                for (ScotlandYard.Transport t : Objects.requireNonNull(Setup.getInstance().graph.edgeValueOrDefault(this.mrX.location(), neighbour, ImmutableSet.of()))) {
                    ...
                    if ((this.mrX.tickets().get(smallTicket) > 0) //if the player has the ticket for this
                            && this.detectives().stream().map(DetSmallPlayer::location).noneMatch(x -> Objects.equals(x, neighbour))) { //and no detectives are here
            ...
            for (DetSmallPlayer player : this.detectives) {   // for each player
    ...
    public Integer didSomeoneWin(Boolean mrXturn) {
        ...
        if (mrXturn) { // if its mr X's turn
            if (this.isTrapped(true) //if mrX is trapped
                    || (this.detectives().stream().map(DetSmallPlayer::location).anyMatch(x -> Objects.equals(x, this.mrX().location())))) {
